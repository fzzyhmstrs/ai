...
class ImbuingTableScreenHandlerV2(
    syncID: Int,
    ...
    private val inventory: SimpleInventory = object : SimpleInventory(13) {
        override fun markDirty() {
            super.markDirty()
            this@ImbuingTableScreenHandlerV2.onContentChanged(this)
        }
    }
                        ...
                        matches.forEachIndexed { index, imbuingRecipe ->
                            ...
                            if (imbuingRecipe.getAugment() != "") {
                                ...
                                if (augment != null) {
                                    ...
                                    if (blA && blB) {
                                        tempResults.add(ModifierResult(imbuingRecipe,false,power))
                                    } else if (blA && !blB) {
                                        tempResults.add(ModifierResult(imbuingRecipe,true,power))
                                    } else {
                                        tempResults.add(EmptyResult())
                                    }
                                } else {
    ...
    interface TableResult{
        ...
        fun isReady(id: Int, player: PlayerEntity, handler: ImbuingTableScreenHandlerV2): Boolean

        fun applyResult(player: PlayerEntity, stack: ItemStack, world: World, pos: BlockPos?, handler: ImbuingTableScreenHandlerV2): Boolean

        fun tooltipList(id: Int, player: PlayerEntity, handler: ImbuingTableScreenHandlerV2): List<Text>
    ...
    class EmptyResult(): TableResult {
        ...
        override fun isReady(id: Int, player: PlayerEntity, handler: ImbuingTableScreenHandlerV2): Boolean {
            return false
        }
        ...
        override fun applyResult(
            ...
            world: World,
            pos: BlockPos?,
            handler: ImbuingTableScreenHandlerV2
        ): Boolean {
            return false
        }

        override fun tooltipList(id: Int, player: PlayerEntity, handler: ImbuingTableScreenHandlerV2): List<Text> {
            return listOf()
        }
    }
    ...
    class ImbuingResult(val recipe: ImbuingRecipe, override val power: Int): TableResult{
        ...
        override fun applyResult(
            ...
            var itemStack3 = stack
            val i = MathHelper.ceil(recipe.getCost() * MathHelper.clamp(AiConfig.altars.imbuingTableDifficultyModifier,0.0F,10.0F))
    ...
    class ModifierResult(val recipe: ImbuingRecipe, val lineageMax: Boolean, override val power: Int): TableResult{
        ...
        override fun buttonStringVisitable(textRenderer: TextRenderer, width: Int): StringVisitable {
            val augId = Identifier(recipe.getAugment())
            val str = ModifierRegistry.getByType<AugmentModifier>(augId)?.getName()?.string ?: Text.translatable("container.disenchanting_table.button.missing_enchantment").toString()
    ...
    class ErrorResult(private val error: List<Text>, private val stringVisitable: StringVisitable, override val power: Int): TableResult{
        override val type: Int = -1
