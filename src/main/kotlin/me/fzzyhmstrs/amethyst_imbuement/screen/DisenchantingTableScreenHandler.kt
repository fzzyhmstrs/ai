...
class DisenchantingTableScreenHandler(
    ...
    override fun onButtonClick(player: PlayerEntity, id: Int): Boolean {
        ...
        when (id) {
            ...
            1 -> {
                if (enchantmentId[id] == -1) return false
                if ((itemStack2.isOf(Items.BOOK) && Registries.ENCHANTMENT.get(enchantmentId[id]) is ScepterAugment) || (itemStack2.isOf(RegisterScepter.EMPTY_SPELL_SCROLL) && Registries.ENCHANTMENT.get(enchantmentId[id]) !is ScepterAugment) || (player.experienceLevel < disenchantCost[0] && !player.abilities.creativeMode)) return false
                context.run { world: World, pos: BlockPos ->
                    removing = true
