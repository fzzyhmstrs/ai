...
class GuardianAugment(weight: Rarity, mxLvl: Int = 1, vararg slot: EquipmentSlot): TotemPassiveAugment(weight,mxLvl, *slot) {
    ...
    override fun specialEffect(user: LivingEntity, level: Int, stack: ItemStack): Boolean {
        ...
        if (RegisterItem.TOTEM_OF_AMETHYST.checkCanUse(stack, world, user as PlayerEntity, 240, TranslatableText("augment_damage.guardian.check_can_use"))) {
            if (RegisterItem.TOTEM_OF_AMETHYST.manaDamage(stack, world, user, 240)) {
                RegisterItem.TOTEM_OF_AMETHYST.burnOutHandler(stack, RegisterEnchantment.GUARDIAN,user, TranslatableText("augment_damage.guardian.burnout"))
            }
