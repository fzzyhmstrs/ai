...
class ImmunityAugment(weight: Rarity, mxLvl: Int = 1, vararg slot: EquipmentSlot): PassiveAugment(weight,mxLvl, *slot) {
    ...
    fun checkAndDamageTrinket(entity: LivingEntity, effect: StatusEffectInstance): Boolean{
        ...
        for (stack in trinkets){
            ...
            if(!RegisterTool.IMBUED_AMULET.checkCanUse(stack,world,entity,damage, AcText.empty())) continue
            RegisterTool.IMBUED_AMULET.manaDamage(stack,world,entity,damage)
        ...
        if (EnchantmentHelper.getLevel(RegisterEnchantment.IMMUNITY,totem1) != 0){
            if(RegisterTool.IMBUED_AMULET.checkCanUse(totem1,world,entity,damage, AcText.empty())){
                RegisterTool.IMBUED_AMULET.manaDamage(totem1,world,entity,damage)
                entity.addStatusEffect(StatusEffectInstance(RegisterStatus.IMMUNITY,60))
                bl = true
            }
        }
        ...
        if (EnchantmentHelper.getLevel(RegisterEnchantment.IMMUNITY,totem2) != 0){
            if(RegisterTool.IMBUED_AMULET.checkCanUse(totem2,world,entity,damage, AcText.empty())){
                RegisterTool.IMBUED_AMULET.manaDamage(totem2,world,entity,damage)
                entity.addStatusEffect(StatusEffectInstance(RegisterStatus.IMMUNITY,60))
                bl = true
            }
        }
