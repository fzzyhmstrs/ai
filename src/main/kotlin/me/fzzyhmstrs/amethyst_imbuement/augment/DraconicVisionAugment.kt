...
class DraconicVisionAugment(weight: Rarity, mxLvl: Int = 1, vararg slot: EquipmentSlot): PassiveAugment(weight,mxLvl, *slot) {
        ...
        for (i in -range..range){
            for (j in -range..range){
                for (k in -range..range){
                    ...
                    if ((bs.block is ExperienceDroppingBlock || bs.isIn(ConventionalBlockTags.ORES)) && !bs.isIn(RegisterTag.DRACONIC_VISION_BLACKLIST)){
    ...
    override fun acceptableItemStacks(): MutableList<ItemStack> {
        return RegisterItem.headbandStacks
    }
