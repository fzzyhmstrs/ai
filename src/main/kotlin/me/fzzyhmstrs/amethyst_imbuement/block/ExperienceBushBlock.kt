...
class ExperienceBushBlock(settings: Settings):SweetBerryBushBlock(settings) {
    ...
    override fun randomTick(state: BlockState, world: ServerWorld, pos: BlockPos, random: Random) {
        val i = state.get(AGE)
        if (i < 3 && random.nextFloat() < AiConfig.blocks.xpBush.growChance.get() && world.getBaseLightLevel(pos.up(), 0) >= 9) {
    ...
    override fun grow(world: ServerWorld, random: Random, pos: BlockPos, state: BlockState) {
        if (random.nextFloat() < AiConfig.blocks.xpBush.bonemealChance.get()) {
