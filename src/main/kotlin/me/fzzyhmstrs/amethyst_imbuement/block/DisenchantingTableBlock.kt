...
class DisenchantingTableBlock(settings: Settings): EnchantingTableBlock(settings) {
    ...
    override fun <T : BlockEntity> getTicker(
        ...
        return if (world.isClient) checkType(
            type, RegisterEntity.DISENCHANTING_TABLE_BLOCK_ENTITY
        ) { wrld: World, pos: BlockPos, state2: BlockState, blockEntity: DisenchantingTableBlockEntity ->
    ...
    override fun onPlaced(
        ...
        if (itemStack.hasCustomName() && blockEntity is DisenchantingTableBlockEntity) {
        ...
        for (i in -2..2) {
            ...
            block1@ while (j <= 2) {
                ...
                for (k in 0..1) {
                    ...
                    if (!world.getBlockState(blockPos).isIn(RegisterTag.PILLARS_TAG)) continue
