...
object RegisterItem {
    ...
    val OPALINE_SCEPTER = ScepterItem(ScepterToolMaterial, FabricItemSettings().group(ItemGroup.COMBAT).maxDamage(250).rarity(Rarity.RARE)).also{ regItem["opaline_scepter"] = it}
    val IRIDESCENT_SCEPTER = ScepterItem(ScepterLvl2ToolMaterial, FabricItemSettings().group(ItemGroup.COMBAT).maxDamage(550).rarity(Rarity.RARE)).also{ regItem["iridescent_scepter"] = it}
    val LUSTROUS_SCEPTER = ScepterItem(ScepterLvl3ToolMaterial, FabricItemSettings().group(ItemGroup.COMBAT).maxDamage(1450).rarity(Rarity.RARE)).also{ regItem["lustrous_scepter"] = it}
    ...
    fun registerAll() {
        ...
        for (k in regItem.keys){
            Registry.register(Registry.ITEM,Identifier("amethyst_imbuement",k), regItem[k])
        }
        regItem.clear()
